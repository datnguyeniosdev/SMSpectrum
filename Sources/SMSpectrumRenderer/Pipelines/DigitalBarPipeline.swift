...
/// Renders the spectrum as a smooth thick line trace through the magnitude
/// curve. Iterates over `effectiveLayers` so each layer can occupy a
/// different horizontal slice with its own gradient and thickness.
final class DigitalBarPipeline: SpectrumPipeline {

    private let pipelineState: MTLRenderPipelineState

    init(library: PipelineLibrary, pixelFormat: MTLPixelFormat) throws {
        let vertex = try library.makeFunction(named: "digitalBarVertex")
        let fragment = try library.makeFunction(named: "digitalBarFragment")

        let descriptor = MTLRenderPipelineDescriptor()
        descriptor.label = "SMSpectrum.DigitalBar"
        ...
        do {
            ...
            throw RenderError.pipelineCreationFailed(stage: "DigitalBar.pipelineState", underlying: error)
        }
    }
    ...
    func encode(
        ...
        guard frame.magnitudes.count >= 2 else {
            throw RenderError.invalidFrame(reason: "DigitalBar requires at least 2 magnitudes.")
        }
        ...
        let magnitudeBuffer = try bufferPool.buffer(forKey: "DigitalBar.magnitudes", byteCount: magnitudeByteCount)
        ...
        for (layerIndex, layer) in layers.enumerated() {
            ...
            let stopBuffer = try bufferPool.buffer(
                forKey: "DigitalBar.stops.\(layerIndex)",
